...
export const payrollRunApiSlice = createApi({
  ...
  endpoints: builder => ({
    ...
    saveDraftSlips: builder.mutation({
      async queryFn({ companyId, branchId, runId, period, slips }) {
        try {
        ...
        try {
          ...
          for (const slip of slips) {
            ...
            batch.set(salaryRef, {
              ...
              // netPay is what the staff actually receives in hand or by transfer.
              // useBranchData sums this as totalSal and routes by payMethod.
              amount:          slip.netPay           || 0,
              grossEarnings:   slip.grossEarnings    || 0,
              totalDeductions: slip.totalDeductions  || 0,
              date:            paidDateStr,
              period:          runData.period        || '',
              // payMethod drives the cash vs bank split in useBranchData:
              //   'cash'  → deducted from handInCash
              //   'bank' | 'bank_transfer' | 'cheque' → deducted from bankExpected
    ...
    deletePayrollRun: builder.mutation({
      async queryFn({ companyId, branchId, runId }) {
        try {
          ...
          batch.delete(runDoc(companyId, branchId, runId))
          await batch.commit()
          return { data: { runId } }
        } catch (e) {
          return { error: { message: e.message } }
