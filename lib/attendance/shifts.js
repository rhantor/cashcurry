// Shift templates.
//
// A branch defines a small set of named shifts (settings.attendance.shifts) and
// each staff member gets one default shift (staff.shiftId). Lateness and OT are
// then measured against that shift instead of a flat basicHoursPerDay.
//
// Everything here is INERT unless `settings.shiftsEnabled` is on AND the staff
// member has a shiftId that resolves — `resolveStaffShift` returns null
// otherwise and the attendance engine keeps its original flat-hours behaviour.
...
export function getShifts(settings) {
  return Array.isArray(settings?.shifts) ? settings.shifts : [];
}

export function shiftsEnabled(settings) {
  return !!settings?.shiftsEnabled && getShifts(settings).length > 0;
}

// The shift a staff member works, or null when shifts don't apply to them.
// Fixed-salary/no-attendance staff (owners) are always exempt.
export function resolveStaffShift(staff, settings) {
  ...
  return getShifts(settings).find((s) => s.id === staff.shiftId) || null;
}
...
//   hours      — paid length: expectedHours override, else span minus unpaid break
export function shiftWindow(dateStr, shift) {
  ...
  const hours = num(shift?.expectedHours, spanHours - unpaidBreakMinutes / 60);

  return { start, end, spanHours, unpaidBreakMinutes, hours: Math.max(0, hours) };
}
...
export function shiftLabel(shift) {
  if (!shift) return "";
  const suffix = isOvernight(shift) ? " ⏭" : "";
  return `${shift.name || "Shift"} (${shift.start}–${shift.end})${suffix}`;
}
