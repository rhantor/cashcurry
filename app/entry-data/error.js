...
export default function EntryDataError({ error, reset }) {
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-rose-50 via-orange-50 to-amber-50 p-6">
      <div className="max-w-md w-full text-center space-y-5 bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-8">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-800">Couldn&apos;t load this page</h1>
          <p className="mt-2 text-sm text-slate-500 leading-relaxed">
            Something went wrong while loading the entry form. Your previous data is safe — retrying usually clears this up.
          </p>
          {error?.message && (
